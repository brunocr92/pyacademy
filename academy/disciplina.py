...
class Discplina(object)

    _ID = 1
    limite_sala = 60

    def __init___(self, nome, descricao, carga, sigla)
    ...
    def matricula(self, alunos):
        '''
        Recebe uma lista de alunos para serem matriculados ou apenas um aluno
        '''
        lista_matricula = []
        novos_alunos = {}
        ...
        for aluno in lista_matricula:
            if isistance (aluno, Aluno):
                if self.ocupar_vaga():
                    Novos_alunos.update({aluno.id: aluno})
            else:
                raise TypeError("Apenas alunos poder sem matriculados em disciplinas")

        self._alunos.update(novos_alunos)
        return True

    def desmatricular(self, **kwargs):
        """
        Para cancelar a matricula de um aluno disciplina.desmatricula(dict_aluno) onde
        dict_aluno = {'id': [1,2,3,...], 'matricula': [1000,10001,1002,...]} onde apenas
        um dos campos do dicionario serao utilizados
        """
        ...
        else: lista_desmatricula = kwargs [tipo_busca]
        ...
        for ID, aluno in self._alunos.items():
            for ids in lista_desmatricula:
                if tipo_busca == 'id' and ID == ids:
                    del self._alunos[ID]
                    self.desocupar_vaga()
                elif tipo busca == 'matricula' and aluno.matricula == ids:
                    del self._alunos[ID]
                    self.desocupar_vaga()
